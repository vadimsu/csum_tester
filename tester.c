...
int main(int argc, char **argv)
{
	...
	memcpy(&message[ETH_ALEN],ifr.ifr_hwaddr.sa_data,ETH_ALEN);
	eh->h_proto = htons(ETH_P_IP);
	iph = (struct iphdr *)&message[14];
	iph->version = 4;
	iph->ihl = 5;
	iph->tot_len = htons(sizeof(message) - (14));
	iph->id = 0;
	iph->frag_off = 0;
	iph->tos = 0;
	iph->ttl = 1;
	...
	if (protocol == 17) {
		...
		uh->len = htons(sizeof(message) - (20+14));
